...
- `sora` 서비스 실지원 — 현재 `picko/video_plan.py` 타입에 플레이스홀더만 존재, 009 범위에서는 제외
...
### 3.1 서비스별 프롬프트 정책 (공식 가이드 반영)

각 서비스는 같은 의도(`ad`/`explainer`/`brand`/`trend`)라도 프롬프트 구성 규칙이 다르다.

- **Luma**
  - 자연어 기반의 구체적 시각 묘사(장면/조명/무드/카메라) 사용
  - 카메라 모션, 스타일 프리셋, 비율을 프롬프트/파라미터에 함께 명시
- **Runway**
  - 대화형/명령형 문장 대신 직접적이고 시각적인 기술 문장 사용
  - 레퍼런스 이미지를 쓸 때는 이미지 설명보다 "원하는 움직임" 중심으로 작성
  - "camera doesn't move" 같은 부정형 지시 대신 긍정형 지시(`static shot`) 사용
- **Pika / Kling / Veo**
  - 서비스별 제어 파라미터(`pikaffect`, `camera_motion`, `generate_audio` 등)를 명시적으로 채움
  - 프롬프트는 장면 구성 + 동작 + 톤(스타일) 순으로 간결하게 작성
- **광고(ad) 공통 규칙**
  - 첫 1-3초 훅(시선 정지 요소) 필수
  - 마지막 3-5초에 명확한 CTA 문구/오버레이/내레이션 포함
  - 9:16 기준 UI safe-zone을 고려해 핵심 텍스트를 배치
...
| 차원 | 가중치 | 평가 기준 |
|------|--------|----------|
| prompt_quality | 25% | 구체성, 시각 묘사, 추상 표현 배제, 서비스별 공식 규칙 준수 |
| structure | 20% | Intent별 필수 구조 준수(특히 ad의 hook → body → cta) |
| brand_alignment | 15% | 톤/비율 일관성 |
| platform_fit | 20% | 길이/비율 제약 + safe-zone 준수 |
