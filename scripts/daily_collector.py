...
핵심 포인트 (각 줄에 하나씩):"""
    ...
    def _export(self, items: list[dict], date: str) -> list[Path]:
        ...
        for item in items:
            ...
            try:
                saved = self.vault.write_note(note_path, content.split("---", 2)[2].strip(), 
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Daily Collector - RSS/크롤링에서 콘텐츠 수집"
    )
    parser.add_argument(
        "--date", "-d",
