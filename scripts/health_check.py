    ...
    def check_sources(self):
        ...
        if not enabled_sources:
            self.results.append(CheckResult(
                ...
                details="Check config/sources.yml"
            ))
            return
        ...
        if accessible == len(enabled_sources[:5]):
            self.results.append(CheckResult(
                name="RSS Sources",
                passed=True,
                message=f"All {accessible} sources accessible"
            ))
        else:
            self.results.append(CheckResult(
                name="RSS Sources",
    ...
    def check_directories(self):
        ...
        if not missing:
            self.results.append(CheckResult(
                name="Directories",
                passed=True,
                message=f"All {len(required_dirs)} directories exist"
            ))
        else:
            self.results.append(CheckResult(
                name="Directories",
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Health Check - 파이프라인 상태 점검"
    )
    parser.add_argument(
        "--json",
        action="store_true",
