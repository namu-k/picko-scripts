...
class RetryManager:
    ...
    def run(self, date: str = None, stage: str = None, dry_run: bool = False) -> dict:
        """
        실패 항목 재시도 실행
        ...
        results = {"date": date, "failed_found": 0, "retried": 0, "succeeded": 0, "still_failed": 0, "errors": []}
    ...
    def _retry_fetch(self, item: dict) -> bool:
        ...
        try:
            ...
            # 성공 시 상태 업데이트
            if item.get("path"):
                self.vault.update_frontmatter(item["path"], {"status": "inbox", "text": text[:3000]})
    ...
    def _retry_nlp(self, item: dict) -> bool:
        ...
        try:
            ...
            self.vault.update_frontmatter(
                item["path"],
                {"status": "inbox", "summary": summary, "tags": tags, "failed_stage": None, "error_message": None},
            )
    ...
    def _retry_embed(self, item: dict) -> bool:
        ...
        try:
            ...
            self.embedder.embed(text)  # noqa: F841  # Embedding is cached by the embedder

            # 임베딩은 별도 캐시에 저장되므로 상태만 업데이트
            self.vault.update_frontmatter(
                item["path"], {"status": "inbox", "embedded": True, "failed_stage": None, "error_message": None}
            )
    ...
    def _retry_score(self, item: dict) -> bool:
        ...
        try:
            ...
            score = scorer.score(
                {"title": meta.get("title", ""), "text": meta.get("summary", ""), "keywords": meta.get("tags", [])}
            )

            self.vault.update_frontmatter(
                item["path"], {"status": "inbox", "score": score.to_dict(), "failed_stage": None, "error_message": None}
            )
...
def main():
    """CLI 엔트리포인트"""
    parser = argparse.ArgumentParser(description="Retry Failed - 실패한 수집 항목 재시도")
    parser.add_argument("--date", "-d", help="대상 날짜 (YYYY-MM-DD, 기본: 오늘)")
    parser.add_argument("--stage", "-s", choices=RetryManager.STAGES, help="특정 단계만 재시도")
    parser.add_argument("--max-attempts", type=int, default=3, help="최대 재시도 횟수 (기본: 3)")
    ...
    results = manager.run(date=args.date, stage=args.stage, dry_run=args.dry_run)
