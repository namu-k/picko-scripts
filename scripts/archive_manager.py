...
class ArchiveManager:
    ...
    def run(self, days: int = 30, clean_cache: bool = False, dry_run: bool = False) -> dict:
        """
        아카이브 처리 실행
        ...
        results = {"threshold_days": days, "scanned": 0, "archived": 0, "cache_cleaned": 0, "errors": []}
    ...
    def _archive_note(self, note_path: Path, meta: dict) -> bool:
        ...
        try:
            ...
            # 상태 업데이트 후 이동
            self.vault.update_frontmatter(note_path, {"status": "archived", "archived_at": datetime.now().isoformat()})
...
def main():
    """CLI 엔트리포인트"""
    parser = argparse.ArgumentParser(description="Archive Manager - 오래된 미승인 콘텐츠 아카이브")
    parser.add_argument("--days", "-d", type=int, default=30, help="N일 이상 지난 항목 대상 (기본: 30)")
    parser.add_argument("--clean-cache", action="store_true", help="관련 임베딩 캐시도 삭제")
    parser.add_argument("--list", action="store_true", help="아카이브 대상 목록만 출력")
