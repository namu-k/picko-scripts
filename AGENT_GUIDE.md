...
# 최대 항목 수 제한 (테스트용)
python -m scripts.daily_collector --max-items 10
```
...
> **Canonical Source**: [docs/DATA_CONTRACTS.md](docs/DATA_CONTRACTS.md)
...
python -m scripts.generate_content -d 2026-02-28
...
python -m scripts.generate_content -t longform packs

# 이미 생성된 항목 재생성
python -m scripts.generate_content --force
python -m scripts.generate_content -f

# 주간 슬롯 컨텍스트 로드
python -m scripts.generate_content --week-of 2026-02-24
python -m scripts.generate_content -w 2026-02-24
```
...
```bash
# 특정 경로 검증 (positional 또는 --path)
python -m scripts.validate_output Content/Longform/
python -m scripts.validate_output --path Content/Longform/
python -m scripts.validate_output -p Content/Longform/

# 재귀 검증
python -m scripts.validate_output Content/ --recursive
python -m scripts.validate_output Content/ -r

# 상세 출력
python -m scripts.validate_output Content/ --recursive --verbose
python -m scripts.validate_output Content/ -r -v
```
...
platform: twitter                    # allowed: twitter | linkedin | newsletter | blog | instagram | youtube
status: published                    # allowed: draft | scheduled | published | cancelled
scheduled_at: 2026-02-28T16:30:00Z   # optional
published_at: 2026-02-28T17:00:00Z
published_url: https://twitter.com/user/status/123456789
platform_post_id: 123456789          # optional, platform-specific id
metrics:
  views: 1234
  likes: 56
  shares: 12                          # platform-agnostic
  comments: 8
  clicks: 234
  impressions: 5678
  platform_specific:                  # optional, platform-specific
    retweets: 12
    quote_tweets: 3
metrics_synced_at: 2026-02-28T18:00:00Z
metrics_source: api                   # allowed: api | manual | unavailable
---
```
