...
> 아래 표는 UI에서 노출하는 **대표 설정 필드 예시**입니다. 실제 값은 `config/config.yml`의 `vault.*`, `llm`/`summary_llm`/`writer_llm`, `embedding.*`, `scoring.*`, `quality.*`, `notification.*` 키와 매핑되며, 내부 키 이름은 약간 다를 수 있습니다.

| 그룹 | 항목 | 타입 | 설명 | UI 컨트롤 |
|------|------|------|------|-----------|
| **Vault** | `root` | string | Vault 루트 경로 | text input |
|  | `inbox` | string | 입력물 경로 | text input |
|  | `digests` | string | 다이제스트 경로 (`vault.inbox` 기준, 예: `Inbox/Inputs/_digests`) | text input |
|  | `explorations` | string | 탐색물 경로 (`vault.inbox` 기반 파생, 예: `Inbox/Explorations`) | text input |
...
| **Quality** | `enabled` | bool | 품질 검사 활성화 (`quality.enabled`) | switch |
|  | `auto_approve_threshold` | number | 자동 승인 임계값 (`quality.final.auto_approve_threshold`) | number input |
...
### 1. 다이제스트 (Digest)
**위치**: `Inbox/Inputs/_digests/YYYY-MM-DD.md`
...
### 1. 콘텐츠 상태
> 실제 Vault 프론트매터(`writing_status`)는 `pending`, `auto_ready`, `manual`, `completed` 등 더 많은 상태를 가지지만, 아래 다이어그램은 UI 설계를 위한 **단순화된 대표 플로우**입니다.
```
pending
    ↓ [auto_ready]
auto_ready
...
│   ├── accounts/           # 계정 프로필
│   └── prompts/            # 프롬프트 템플릿
├── vault/                  # 실제론 `config.vault.root` (예: mock_vault) 아래 구조
│   ├── Inbox/Inputs/       # 수집된 콘텐츠
│   ├── Inbox/Inputs/_digests/ # 다이제스트 (config.vault.digests)
│   ├── Inbox/Explorations/ # 탐색물 (파생 경로)
│   ├── Content/            # 생성된 콘텐츠
│   ├── Multimedia/         # 미디어 관련
│   └── Quality/            # 품질 검사
└── cache/
